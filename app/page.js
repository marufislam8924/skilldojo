...
export default function Home() {
  ...
  const courses = [
    { key:"hiragana", kana:"あ", title:"Hiragana Complete", desc:"Master all 46 hiragana with AI voice.", lessons:21, level:"Beginner", bg:"#fff0f0", live:true },
    ...
    { key:"vocab",    kana:"言", title:"Daily Vocabulary",  desc:"500+ essential words by topic.", lessons:30, level:"Intermediate", bg:"#f0fff4", live:false },
  ...
  return (
    <main className={styles.main}>
      <nav className={styles.nav}>
        ...
        <a href="https://youtube.com/@skilldojo-b2t" target="_blank" className={styles.ytLink}>▶ YouTube</a>
      </nav>

      <section className={styles.hero}>
        <div className={styles.heroText}>
          <div className={styles.heroTag}>🇯🇵 Free Japanese Course</div>
          <h1 className={styles.heroTitle}>Learn Japanese<br/>the <em>Right</em> Way.</h1>
          <p className={styles.heroDesc}>SkillDojo teaches real Japanese — Hiragana, Katakana, vocabulary and conversation — step by step. No fluff.</p>
          <div className={styles.heroBtns}>
            <button className={styles.btnPrimary} onClick={() => router.push("/hiragana")}>Start for Free →</button>
            <a href="https://youtube.com/@skilldojo-b2t" target="_blank" className={styles.btnSecondary}>Watch on YouTube</a>
          </div>
        </div>
        ...
        <div className={styles.coursesGrid}>
          {courses.map((c) => (
            <div key={c.key} className={styles.courseCard}
              ...
              <div className={styles.courseThumb} style={{background:c.bg}}>{c.kana}</div>
              <div className={styles.courseBody}>
                ...
                {!c.live && <div className={styles.comingSoon}>Coming Soon</div>}
              </div>
            </div>
          ))}
        </div>
      ...
      <footer className={styles.footer}>
        <div>
          <div className={styles.footerLogo}>Skill<span style={{color:"var(--red)"}}>Dojo</span> 道場</div>
          <p style={{color:"#7a7a7a",fontSize:"0.85rem",marginTop:"6px"}}>Free Japanese education for everyone.</p>
        </div>
